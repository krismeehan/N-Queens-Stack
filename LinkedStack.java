...
public class LinkedStack<E> implements Cloneable
{
   ...
   * @param - number
   *  The index of which to return a value
   * <dt><b>Precondition:</b><dd>
   *   The number is not outside the size of the stack
   * <dt><b>Postcondition:</b><dd>
   *   The return value is the value at the specified index
   * @exception NoSuchElementException
   *   Indicates that the index is out of the range of the stack.
   **/   
   public E itemAt(int num)   
   {
      ...
      // push all values above desired value to temporary stack   
      for(int i = 0; i < num; i++)
      {
         temp.push(pop());  
      }
      ...
      // place items moved back on the stack
      for(int j = 0; j < num; j++)
      {
         push((E)temp.pop());  
      }
